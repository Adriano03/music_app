...
class _MusicPlayerMusicDurationWidgetState extends State<MusicPlayerMusicDurationWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(
      ...
      builder: (context, snapshot) {
        final seconds =
            snapshot.data?.inSeconds ?? musicPlayerController.currentMusicDuration.value;
        return Column(
          children: [
            Obx(
              () => Slider(
                value: (sliderValueRx.value ?? seconds / widget.duration).clamp(0, 1),
                onChanged: (double sliderValue) async {
                  await musicPlayerController.seek((sliderValue * widget.duration).floor());
            ...
            Row(
              ...
              children: [
                ...
                TextWidget.small('-${(widget.duration - seconds).fromSecondsToMinutesString}'),
              ],
            ),
          ],
        );
